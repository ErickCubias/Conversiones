...
const Longitud = () => {
  ...
  const [toUnit, setToUnit] = useState('pies');
  ...
  const convertLength = async () => {
    ...
    let convertedValue;
    if (fromUnit === 'metros' && toUnit === 'pies') {
      convertedValue = numericValue * 3.28084; // Metros a Pies
    } else if (fromUnit === 'pies' && toUnit === 'metros') {
      convertedValue = numericValue / 3.28084; // Pies a Metros
    }

    const message = `Longitud: 
